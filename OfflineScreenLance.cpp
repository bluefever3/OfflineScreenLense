...
int APIENTRY wWinMain(_In_ HINSTANCE hInstance,
    ...
    _In_ int       nCmdShow)
{
    HMODULE hOrtDll = LoadLibraryW(L"onnxruntime.dll");
    if (hOrtDll == nullptr) {
        // ... (hata kodu)
        return 1;
    }
    else {
        OutputDebugStringW(L"onnxruntime.dll loaded successfully via LoadLibraryW.");
    }

    // CppWinRT başlatması (eğer varsa, Ort::Env'den önce olmalı)
    // winrt::init_apartment(winrt::apartment_type::single_threaded);

    try {
        // Ort::Env'i burada, wWinMain içinde yerel bir değişken olarak oluşturun
        Ort::Env env(ORT_LOGGING_LEVEL_WARNING, "ocr-translator-env");
        MessageBoxW(nullptr, L"Ana Projede ONNX Env wWinMain içinde oluşturuldu!", L"Başarılı", MB_OK);

        // ... (programınızın geri kalanı env'i kullanabilir) ...

    }
    catch (const Ort::Exception& e) {
        std::string s = e.what();
        std::wstring ws(s.begin(), s.end());
        MessageBoxW(nullptr, (L"Ana Proje ONNX İstisnası: " + ws).c_str(), L"Hata", MB_OK | MB_ICONERROR);
        if (env == nullptr) { // Bu kontrol aslında Ort::Env constructor'ı hata verirse diye, ama e.what() daha iyi bilgi verir.
            OutputDebugStringW(L"Ort::Env constructor failed in wWinMain.");
        }
        // ... (gerekirse programdan çıkın)
    }
